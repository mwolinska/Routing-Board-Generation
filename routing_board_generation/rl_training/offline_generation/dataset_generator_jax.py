...
class BoardDatasetGeneratorJAX(Generator):
    def __init__(
        ...
        if board_name == "offline_seed_extension":
            ...
            self.board_generator_call = jax.jit(
                self.board_generator.generate_starts_ends
            )
            ...
            self.board_generator = ParallelRandomWalkBoard(
                grid_size, grid_size, num_agents
            )
            self.board_generator_call = jax.jit(self.board_generator.generate_board)

        heads, targets, solved_boards = self.generate_n_boards(
            jax.random.PRNGKey(0), number_of_boards, generate_solved_boards
        )
        ...
        for i in range(n_boards):
            ...
            if self.board_name == "offline_seed_extension":
                if generate_solved_boards:
                    ...
                    solved_board = solved_board_call(
                        key,
                        self.randomness,
                        self.two_sided,
                        self.extension_iterations,
                        self.extension_steps,
                    )
                    solved_boards_list.append(solved_board)
                else:
                    heads_for_board, targets_for_board = self.board_generator_call(
                        key,
                        self.randomness,
                        self.two_sided,
                        self.extension_iterations,
    ...
    def __call__(self, key: chex.PRNGKey) -> State:
        ...
        which_board = jax.random.randint(
            key, shape=(), minval=0, maxval=len(self.heads)
        )
...
if __name__ == "__main__":
    test = BoardDatasetGeneratorJAX(
        10, 5, number_of_boards=10, board_name="offline_seed_extension"
    )
