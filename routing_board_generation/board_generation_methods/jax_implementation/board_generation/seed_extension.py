# # from ic_routing_board_generation.board_generator.numpy_data_model.abstract_board import \
# #     AbstractBoard
...
from routing_board_generation.board_generation_methods.jax_implementation.utils.grid_utils import \
    optimise_wire
from routing_board_generation.board_generation_methods.jax_implementation.utils.post_processor_utils_jax import \
    position_to_wire_num_jax, training_board_from_solved_board_jax, extend_wires_jax
...
# INVALID = jnp.array(
#     (-999, -999))  # Encoding for an invalid 2D position for lists that must be of constant length
# INVALID4 = jnp.array([(-999, -999), (-999, -999), (-999, -999),
#                       (-999, -999)])  # Encoding for list of four invalid positions
#
#
# # @jax.disable_jit()
# # @jax.jit
...
#         max_num = jax.lax.select(num_agents > rows * cols // 3, jnp.array(rows * cols // 3, int),
#                                  num_agents)
...
#             row, col, board_layout, _ = jax.lax.while_loop(position_cond_func, position_body_func,
#                                                            (row, col, board_layout, key))
...
#             neighbor_pos, _ = jax.lax.while_loop(neighbor_cond_func, neighbor_body_func,
#                                                  (neighbor_pos, key))
...
#             board_layout = self.extend_wires_jax(board_layout, extkey, randomness, two_sided,
#                                                  extension_steps)
#
...
#                 board_layout, _ = jax.lax.fori_loop(0, self._wires_on_board,
#                                                     optimise_wire_loop_func, carry)
...
#             (Array) : 2D layout of the board with all wirings encoded
#         """
#         board_layout = self.return_solved_board(key, randomness, two_sided, extension_iterations,
#                                                 extension_steps)
...
#                     poss_extension_list = self.get_open_adjacent_cells_jax(current_pos,
#                                                                            board_layout)
#
...
#                         num_adjacencies = self.num_wire_adjacencies_jax(cell_pos, current_wire_num,
#                                                                         board_layout)
#                         cell_or_invalid = jax.lax.select(num_adjacencies > 1, INVALID, cell_pos)
#                         return cell_or_invalid
#
#                     poss_extension_list = jax.vmap(invalidate_func, in_axes=(0, None, None))(
#                         poss_extension_list, current_wire_num, board_layout)
#
...
#                     priority_neighbor = (
#                     row + (row - prev_neighbor[0]), col + (col - prev_neighbor[1]))
...
#                     is_extendable = (two_sided & (
#                                 (cell_type == STARTING_POSITION) | (cell_type == TARGET))) | \
#                                     (~two_sided & (cell_type == TARGET))
#                     poss_extension_list = jax.lax.select(is_extendable, poss_extension_list,
#                                                          INVALID4)
#
...
#                                                              (poss_extension_list, extension_pos,
#                                                               key))
#
...
#                         jax.vmap(priority_found_func)(poss_extension_list))
#
...
#     # @jax.jit
#     # This method is used by the extend_wires_jax method
#     def num_wire_adjacencies_jax(self, input_pos: jnp.array, wire_num: Array,
#                                  board_layout: Array) -> Array:
...
#         num_adjacencies = 0
#         # Check above
#         wire_adjacent = self.position_to_wire_num_jax(jnp.array((input_pos[0] - 1, input_pos[1])),
#                                                       board_layout)
...
#         num_adjacencies = num_adjacencies + jax.lax.select(wire_adjacent == wire_num, 1, 0)
#         # Check left
#         wire_adjacent = self.position_to_wire_num_jax(jnp.array((input_pos[0], input_pos[1] - 1)),
#                                                       board_layout)
#         num_adjacencies = num_adjacencies + jax.lax.select(wire_adjacent == wire_num, 1, 0)
#         # Check right
#         wire_adjacent = self.position_to_wire_num_jax(jnp.array((input_pos[0], input_pos[1] + 1)),
...
#         adjacent_cell = jax.lax.select(
#             (adjacent_x >= 0) & (board_layout[adjacent_x, adjacent_y] == EMPTY),
#             jnp.array((adjacent_x, adjacent_y)), INVALID)
#         adjacent_list.append(adjacent_cell)
#         # Check left
#         adjacent_x, adjacent_y = input_pos[0], input_pos[1] - 1
#         adjacent_cell = jax.lax.select(
#             (adjacent_y >= 0) & (board_layout[adjacent_x, adjacent_y] == EMPTY),
#             jnp.array((adjacent_x, adjacent_y)), INVALID)
#         adjacent_list.append(adjacent_cell)
#         # Check below
#         adjacent_x, adjacent_y = input_pos[0] + 1, input_pos[1]
#         adjacent_cell = jax.lax.select(
#             (adjacent_x < rows) & (board_layout[adjacent_x, adjacent_y] == EMPTY),
#             jnp.array((adjacent_x, adjacent_y)), INVALID)
#         adjacent_list.append(adjacent_cell)
#         # Check right
#         adjacent_x, adjacent_y = input_pos[0], input_pos[1] + 1
#         adjacent_cell = jax.lax.select(
#             (adjacent_y < cols) & (board_layout[adjacent_x, adjacent_y] == EMPTY),
#             jnp.array((adjacent_x, adjacent_y)), INVALID)
#         adjacent_list.append(adjacent_cell)
#         #  There will always be four items in the list but some of them might be 'INVALID'
...
class SeedExtensionBoard(AbstractBoard):
    ...
    def return_solved_board(self, key: PRNGKey, randomness: float = 0.0, two_sided: bool = True,
        ...
        def ext_iteration_body(carry):
            ...
            key, extkey, optkey = jax.random.split(key, 3)
            board_layout = extend_wires_jax(board_layout, extkey, randomness, two_sided,
                                            extension_steps)
            # print(board_layout)
            optkeys = jax.random.split(optkey, self._wires_on_board)
            ...
            carry = (board_layout, optkeys)
            board_layout, _ = jax.lax.fori_loop(0, self._wires_on_board, optimise_wire_loop_func,
                                                carry)
        ...
        iteration_num = 0
        carry = (board_layout, key, iteration_num, converged)
        board_layout, key, iteration_num, converged = jax.lax.while_loop(ext_iterations_cond,
                                                                         ext_iteration_body, carry)
        # END WHILE LOOP
