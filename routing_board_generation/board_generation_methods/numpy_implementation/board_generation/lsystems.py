...
import jax
import chex
from routing_board_generation.board_generation_methods.jax_implementation.utils import grid_utils
...
class LSystemBoard(AbstractBoard):
    ...
    def initialise_starting_board(self,
                                  key: random.PRNGKey) -> List[LSystemsAgent]:
        ...
        for i in range(self.num_agents):
            ...
            agents.append(
                stack_push_head(new_agent, np.asarray([row_indices[i], col_indices[i]])))  # initial coords given here
        return agents
    ...
    def find_empty_neighbours(self, loc: Tuple[int, int], agents: List[LSystemsAgent]) -> Tuple[chex.Array, chex.Array]:
    ...
    def growth_neighbour_loc(self, key: random.PRNGKey, neighbour_locs: np.ndarray,
    ...
    def grow_agent(self, side: int, loc: np.ndarray, agent: LSystemsAgent) -> LSystemsAgent:
        """Given a location tuple and an Agent, grow the Agent in the given direction."""
        grown_agent = jax.lax.cond(side == 0, lambda _: stack_push_head(agent, loc),
                                   lambda _: stack_push_tail(agent, loc), None)
        return grown_agent

    def push(self, key: random.PRNGKey, agent: LSystemsAgent, agents: List[LSystemsAgent]) -> LSystemsAgent:
        """Pushes (grows) an Agent in a random direction."""
        side = self.heads_or_tails(key)  # stochastic
        pointer = jax.lax.select(side == 0, (agent.head_insertion_index - 1) % self.max_size,
                                 (agent.tail_insertion_index + 1) % self.max_size)  # deterministic
        ...
        agent = jax.lax.cond(loc[0] >= 0, lambda _: self.grow_agent(side, loc, agent), lambda _: agent,
                             None)
        return agent
    ...
    def pull(self, key: random.PRNGKey, agent: LSystemsAgent, agents: List[LSystemsAgent]) -> LSystemsAgent:
    ...
    def fill(self,
             key: random.PRNGKey,
             agents: List[LSystemsAgent]) -> Tuple[random.PRNGKey, List[LSystemsAgent]]:
    ...
    def fill_unsolved_board(self, agents: LSystemsAgent) -> chex.Array:
        ...
        for i in range(len(agents)):
            for j in range(len(agents[0].data)):
                ...
                board = jax.lax.select(self.is_head(left, middle, right),
                                       board.at[tuple(agents[i].data[j])].set(3 * i + 2), board)
                board = jax.lax.select(self.is_tail(left, middle, right),
                                       board.at[tuple(agents[i].data[j])].set(3 * i + 3), board)
        return np.asarray(board, dtype=int)

    def is_empty_self_and_neighbours(self, idx: np.ndarray, agent: LSystemsAgent) -> bool:
        """Tests if a location and its neighbours are empty."""
        return np.all(-1 == agent.data[(idx - 1) % self.max_size]), \
    ...
    def fill_solved_board(self, agents: List[LSystemsAgent]) -> np.ndarray:
        ...
        for i in range(len(agents)):
            for j in range(len(agents[0].data)):
                left, middle, right = self.is_empty_self_and_neighbours(j, agents[i])
                board = jax.lax.select(self.is_body(left, middle, right),
                                       board.at[tuple(agents[i].data[j])].set(3 * i + 1), board)
                board = jax.lax.select(self.is_head(left, middle, right),
                                       board.at[tuple(agents[i].data[j])].set(3 * i + 2), board)
                board = jax.lax.select(self.is_tail(left, middle, right),
                                       board.at[tuple(agents[i].data[j])].set(3 * i + 3), board)
    ...
    def declutter(self, key: random.PRNGKey, wire_num: int, agents: List[LSystemsAgent]):
    ...
    def wire_extend(self, key: random.PRNGKey, agents: List[LSystemsAgent]) -> jax.Array:
