...
import jax
import chex
from routing_board_generation.board_generation_methods.jax_implementation.utils import (
    grid_utils,
)
from routing_board_generation.board_generation_methods.jax_implementation.utils.post_processor_utils_jax import (
    extend_wires_jax,
)
from routing_board_generation.board_generation_methods.jax_implementation.data_model.deque import (
    LSystemsAgent,
    create_stack,
    stack_push_head,
    stack_push_tail,
...
class LSystemBoard(AbstractBoard):
    def __init__(
        self,
        rows: int,
        cols: int = None,
        num_agents: int = None,
        n_iters: int = 5,
        pushpullnone_ratios: Tuple[Numeric] = (2, 0.5, 1),
    ) -> None:
    ...
    def initialise_starting_board(self, key: random.PRNGKey) -> List[LSystemsAgent]:
        ...
        indices = random.permutation(key, np.arange(self.rows * self.cols))[
            : self.num_agents
        ]
        ...
        for i in range(self.num_agents):
            ...
            agents.append(
                stack_push_head(new_agent, np.asarray([row_indices[i], col_indices[i]]))
            )  # initial coords given here
        return agents
    ...
    def find_empty_neighbours(
        self, loc: Tuple[int, int], agents: List[LSystemsAgent]
        ...
        v2 = jax.lax.select(
            j != self.cols - 1, np.equal(board[i, j + 1], 0), False
        )  # east
        v3 = jax.lax.select(
            i != self.rows - 1, np.equal(board[i + 1, j], 0), False
        )  # south
    ...
    def growth_neighbour_loc(
        self, key: random.PRNGKey, neighbour_locs: np.ndarray, neighbours: np.ndarray
    ) -> np.ndarray:
        """Chooses the location of the next growth of an Agent according to free space in neighbouring cells."""
        which_neighbour = jax.lax.select(
            neighbours.sum() > 0, jax.random.choice(key, 4, (), p=neighbours), -1
        )
        loc = jax.lax.select(
            which_neighbour >= 0, neighbour_locs[which_neighbour], np.asarray((-1, -1))
        )
        return loc

    def grow_agent(
        self, side: int, loc: np.ndarray, agent: LSystemsAgent
    ) -> LSystemsAgent:
        """Given a location tuple and an Agent, grow the Agent in the given direction."""
        grown_agent = jax.lax.cond(
            side == 0,
            lambda _: stack_push_head(agent, loc),
            lambda _: stack_push_tail(agent, loc),
            None,
        )
        return grown_agent

    def push(
        self, key: random.PRNGKey, agent: LSystemsAgent, agents: List[LSystemsAgent]
    ) -> LSystemsAgent:
        """Pushes (grows) an Agent in a random direction."""
        side = self.heads_or_tails(key)  # stochastic
        pointer = jax.lax.select(
            side == 0,
            (agent.head_insertion_index - 1) % self.max_size,
            (agent.tail_insertion_index + 1) % self.max_size,
        )  # deterministic
        loc = agent.data[pointer]
        neighbours, neighbour_locs = self.find_empty_neighbours(
            loc, agents
        )  # deterministic
        loc = self.growth_neighbour_loc(key, neighbour_locs, neighbours)
        agent = jax.lax.cond(
            loc[0] >= 0,
            lambda _: self.grow_agent(side, loc, agent),
            lambda _: agent,
            None,
        )
        return agent

    def shrink_agent(self, side: int, agent: LSystemsAgent) -> LSystemsAgent:
    ...
    def do_nothing(self, side: int, agent: LSystemsAgent) -> LSystemsAgent:
        return agent
    ...
    def pull(
        ...
        agent = jax.lax.cond(
            agent_length > 1, self.shrink_agent, self.do_nothing, side, agent
        )
        return agent

    def fill(
        self, key: random.PRNGKey, agents: List[LSystemsAgent]
    ) -> Tuple[random.PRNGKey, List[LSystemsAgent]]:
        return jax.lax.while_loop(self.are_agents_good, self.fill_method, (key, agents))

    def are_agents_good(self, inp) -> bool:
        key, agents = inp
        is_board_good = np.asarray(
            [np.count_nonzero(agent.data[:, 0] + 1) > 1 for agent in agents]
        )
        all_good = np.all(is_board_good)
        return ~all_good

    def fill_method_inner(self, i, inp) -> Tuple[random.PRNGKey, List[LSystemsAgent]]:
        chosen_key, agents = inp
        new_agents = agents
        for i in range(len(agents)):
            key, _ = random.split(chosen_key)
            action = jax.random.choice(
                key, 3, (), p=np.asarray(self.pushpullnone_ratios)
            )
            nagent = jax.lax.cond(
                action == 0, self.push, self.pull, key, agents[i], new_agents
            )
            new_agents[i] = nagent
        return key, new_agents

    def fill_method(self, inp) -> Tuple[random.PRNGKey, List[LSystemsAgent]]:
        return jax.lax.fori_loop(0, self.n_iters, self.fill_method_inner, inp)

    def fill_unsolved_board(self, agents: LSystemsAgent) -> chex.Array:
        ...
        for i in range(len(agents)):
            for j in range(len(agents[0].data)):
                ...
                board = jax.lax.select(
                    self.is_head(left, middle, right),
                    board.at[tuple(agents[i].data[j])].set(3 * i + 2),
                    board,
                )
                board = jax.lax.select(
                    self.is_tail(left, middle, right),
                    board.at[tuple(agents[i].data[j])].set(3 * i + 3),
                    board,
                )
        return np.asarray(board, dtype=int)

    def is_empty_self_and_neighbours(
        self, idx: np.ndarray, agent: LSystemsAgent
    ) -> bool:
        """Tests if a location and its neighbours are empty."""
        return (
            np.all(-1 == agent.data[(idx - 1) % self.max_size]),
            np.all(-1 == agent.data[idx]),
            np.all(-1 == agent.data[(idx + 1) % self.max_size]),
        )
    ...
    def fill_solved_board(self, agents: List[LSystemsAgent]) -> np.ndarray:
        ...
        for i in range(len(agents)):
            for j in range(len(agents[0].data)):
                left, middle, right = self.is_empty_self_and_neighbours(j, agents[i])
                board = jax.lax.select(
                    self.is_body(left, middle, right),
                    board.at[tuple(agents[i].data[j])].set(3 * i + 1),
                    board,
                )
                board = jax.lax.select(
                    self.is_head(left, middle, right),
    ...
    def return_training_board(self, key: random.PRNGKey) -> chex.Array:
        agents = self.initialise_starting_board(key)
        modified_agents = self.fill(key, agents)
    ...
    def declutter(
        self, key: random.PRNGKey, wire_num: int, agents: List[LSystemsAgent]
    ):
        ...
        return grid_utils.thin_wire(
            key, self.fill_solved_board(agents), wire_start, wire_end, wire_num
        )
...
if __name__ == "__main__":
    ...
    board = LSystemBoard(rows=5, cols=5, num_agents=5)
    agents = board.initialise_starting_board(key)
    new_agents = board.fill(key, agents)
