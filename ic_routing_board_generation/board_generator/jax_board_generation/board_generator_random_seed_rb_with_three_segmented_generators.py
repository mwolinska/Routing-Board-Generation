...
class RandomSeedBoard(AbstractBoard):
    ...
    def return_solved_board(self, key: PRNGKey, randomness: float = 0.0, two_sided: bool = True,
        ...
        """
        # While Loop: Loop through specified num of optimisation/extension iterations
        # (checking for convergence is written in as a future desired feature.)
...
class RandomSeedOnlyBoard(AbstractBoard):
...
#@jax.disable_jit()
#@jax.jit
class ExtensionsOnlyBoard(AbstractBoard):
    """ The boards are 2D arrays of wiring routes on a printed circuit board.
...
#@jax.disable_jit()
#@jax.jit
class OptimiseOnlyBoard(AbstractBoard):
    """ The boards are 2D arrays of wiring routes on a printed circuit board.
