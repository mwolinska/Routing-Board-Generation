...
from ic_routing_board_generation.board_generator.jax_board_generation.grid_jax import (
    Grid,
)
...
class SequentialRandomWalk:
    ...
    def available_cells(self, grid: chex.Array, cell: int) -> chex.Array:
        ...
        _, touching_cells_mask = jax.lax.scan(
            self.is_cell_touching_self, (grid, wire_id), adjacent_cells
        )
        ...
        return jnp.hstack(
            (available_cells, jnp.full(self._rows - len(available_cells) + 1, -1))
        )

    def is_cell_free(self, grid: chex.Array, cell: int) -> Tuple[chex.Array, bool]:
    ...
    def one_step(
        ...
        key, subkey = jax.random.split(key)
        cell = (
            wire.path[wire.insertion_index - 1][0] * self._cols
            + wire.path[wire.insertion_index - 1][1]
        )
        ...
        grid = grid.at[
            wire.path[wire.insertion_index - 1][0],
            wire.path[wire.insertion_index - 1][1],
        ].set(3 * wire_id + PATH)
    ...
    def walk_randomly(
        ...
        def single_step(
            ...
            random_walk_tuple = jax.lax.cond(
                can_step, self.one_step, lambda x: x, random_walk_tuple
            )
            return (random_walk_tuple, moved)

        random_walk_tuple, moved = jax.lax.fori_loop(
            0, max_length, single_step, (random_walk_tuple, moved)
        )
    ...
    def add_agents(
        ...
        for wire_id in range(self._num_agents):
            ...
            random_walk_tuple, can_start = self.pick_start(
                subkey, board, wire_id, max_length
            )
            # If possible, try to walk randomly
            random_walk_tuple, moved = jax.lax.cond(
                can_start, self.walk_randomly, lambda x: (x, False), random_walk_tuple
            )
    ...
    def generate(self, key: chex.PRNGKey) -> chex.Array:
        ...
        def try_to_generate(
            ...
            def generate_new_board() -> Tuple[Tuple[chex.Array, bool], bool]:
                ...
                new_board, new_success = self.add_agents(
                    key, empty_board, max_length_int - i
                )
            ...
            updated_board, updated_success = jax.lax.cond(
                jnp.logical_not(success), generate_new_board, keep_successful_board
            )
        ...
        max_length_int = jax.lax.convert_element_type(
            start_max_length, jnp.int32
        ).astype(int)
        ...
        final_board, success = jax.lax.fori_loop(
            1, max_length_int + 1, body_fun_partial, init_state
        )
