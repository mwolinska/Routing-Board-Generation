...
INVALID = jnp.array(
    (-999, -999)
)  # Encoding for an invalid 2D position for lists that must be of constant length
INVALID4 = jnp.array(
    [(-999, -999), (-999, -999), (-999, -999), (-999, -999)]
)  # Encoding for list of four invalid positions


def extend_wires_jax(
    board_layout: chex.Array,
    key: chex.PRNGKey,
    ...
    Args:
        board_layout (chex.Array): 2D layout of the encoded board (before wire extension)
        key (chex.PRNGKey): a random key to use for randomly selecting directions (as needed)
    ...
    Returns:
        (chex.Array): 2D layout of the encoded board (after wire extension)
    """
    ...
    def extend_body(carry: Tuple[chex.Array, chex.Array, chex.PRNGKey, int]) -> Tuple:
        ...
        def row_func(
            row: int, carry: Tuple[chex.Array, chex.PRNGKey]
        ) -> Tuple[chex.Array, chex.PRNGKey]:
            """TODO: Add docstring"""
            board_layout, key = carry

            def col_func(
                col: int, carry: Tuple[chex.Array, chex.PRNGKey]
            ) -> Tuple[chex.Array, chex.PRNGKey]:
                """"""
                board_layout, key = carry
                ...
                poss_extension_array = get_open_adjacent_cells_jax(
                    current_pos, board_layout
                )
                ...
                def invalidate_func(cell_pos, current_wire_num, board_layout):
                    num_adjacencies = num_wire_adjacencies_jax(
                        cell_pos, current_wire_num, board_layout
                    )
                    cell_or_invalid = jax.lax.select(
                        num_adjacencies > 1, INVALID, cell_pos
                    )
                    return cell_or_invalid

                poss_extension_array = jax.vmap(
                    invalidate_func, in_axes=(0, None, None)
                )(poss_extension_array, current_wire_num, board_layout)
                ...
                priority_neighbor = (
                    row + (row - prev_neighbor[0]),
                    col + (col - prev_neighbor[1]),
                )
                ...
                is_extendable = (
                    two_sided
                    & ((cell_type == STARTING_POSITION) | (cell_type == TARGET))
                ) | (~two_sided & (cell_type == TARGET))

                poss_extension_array = jax.lax.select(
                    is_extendable, poss_extension_array, INVALID4
                )

                # If there are no valid options to extend to, stop extending
                stop_extension = jnp.array_equal(poss_extension_array, INVALID4)
                poss_extension_array = jax.lax.select(
                    stop_extension,
                    jnp.array([current_pos, current_pos, current_pos, current_pos]),
                    poss_extension_array,
                )

                # Pick a random selection from the possible extensions until we pick a valid one.
                def choice_body_func(
                    carry: Tuple[chex.Array, chex.Array, chex.PRNGKey]
                ) -> Tuple[chex.Array, chex.Array, chex.PRNGKey]:
                    """Choose a random extension from the list of possible extensions"""
                    poss_extension_array, extension_pos, key = carry
                    key, choice_key = jax.random.split(key)
                    extension_pos = jax.random.choice(choice_key, poss_extension_array)
                    carry = (poss_extension_array, extension_pos, key)
                    return carry
                ...
                _, extension_pos, _ = jax.lax.while_loop(
                    choice_cond_func,
                    choice_body_func,
                    (poss_extension_array, extension_pos, key),
                )
                ...
                priority_neighbor_available = jnp.any(
                    jax.vmap(priority_found_func)(poss_extension_array)
                )
                ...
                extension_pos = jax.lax.select(
                    priority_neighbor_available & ~use_random,
                    priority_neighbor,
                    extension_pos,
                )

                # Extend head/target into new cell
                board_layout = board_layout.at[extension_pos[0], extension_pos[1]].set(
                    board_layout[current_pos[0], current_pos[1]]
                )
                cell_type = position_to_cell_type_jax(current_pos, board_layout)
                ...
                board_layout = board_layout.at[current_pos[0], current_pos[1]].add(
                    cell_type_offset
                )
...
def get_previous_neighbor_jax(
    input_pos: jnp.array, board_layout: chex.Array
) -> jnp.array:
    ...
    Args:
        ...
        board_layout (chex.Array): 2D layout of board with wires encoded
    ...
    def matching_wire_func(i, output_pos):
        ...
        is_prev_neighbor = (
            (neighbor_wire_num == input_wire_num)
            & (neighbor_position[0] >= 0)
            & (neighbor_position[0] < board_layout.shape[0])
            & (neighbor_position[1] >= 0)
            & (neighbor_position[1] < board_layout.shape[1])
        )
...
def get_neighbors_same_wire_jax(
    input_pos: jnp.array, board_layout: chex.Array
) -> jnp.array:
...
def num_wire_adjacencies_jax(
    input_pos: jnp.array, wire_num: chex.Array, board_layout: chex.Array
) -> chex.Array:
    ...
    Args:
        ...
        board_layout (chex.Array): 2D array of board with wires encoded
...
def get_open_adjacent_cells_jax(
    input_pos: jnp.array, board_layout: chex.Array
) -> jnp.array:
    """Returns an array of open cells adjacent to the input cell.

    Args:
        input_pos (chex.Array, Tuple[int, int]): The 2D position of the input cell to search adjacent to.
        board_layout (chex.Array): 2D layout of the board with wires encoded
    ...
    def check_direction(direction: jnp.array) -> jnp.array:
        adjacent_x, adjacent_y = input_pos + direction
        in_bounds = jnp.logical_and(
            jnp.logical_and(adjacent_x >= 0, adjacent_x < rows),
            jnp.logical_and(adjacent_y >= 0, adjacent_y < cols)
        )
        return jax.lax.select(
            in_bounds & (board_layout[adjacent_x, adjacent_y] == EMPTY),
            jnp.array((adjacent_x, adjacent_y)),
            INVALID,
        )

    adjacent_array = jax.vmap(check_direction)(directions)
    return adjacent_array


def position_to_wire_num_jax(
    position: jnp.array, board_layout: chex.Array
) -> chex.Array:
    ...
    wire_num = jax.lax.select(
        (0 <= row) & (row < rows) & (0 <= col) & (col < cols),
        cell_encoding_to_wire_num_jax(cell_encoding),
        -1,
    )
    return wire_num


def cell_encoding_to_wire_num_jax(cell_encoding: chex.Array) -> chex.Array:
...
def position_to_cell_type_jax(
    ...
    Args:
        ...
        board_layout (chex.Array): 2D layout of the encoded board

    Returns:
        (int) : The type of cell (0-3) as detailed above.
    """
    cell_encoding = board_layout[position[0], position[1]]
    cell_type = jax.lax.select(
        cell_encoding == 0, cell_encoding, ((cell_encoding - 1) % 3) + 1
    )
    return cell_type


def training_board_from_solved_board_jax(board_layout: chex.Array) -> chex.Array:
    """Zero out and PATH cells to convert from a solved board to a training board.

    Args:
        board_layout (chex.Array): 2D layout of the encoded solved board

    Returns:
        (chex.Array): 2D layout of the encoded training board
    """
    rows, cols = board_layout.shape
