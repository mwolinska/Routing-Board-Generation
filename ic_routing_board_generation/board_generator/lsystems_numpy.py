...
class LSystemBoardGen:
    ...
    def generate_boards(self, n_boards:int=10000, n_steps:int=20, pushpullnone_ratios:List[Numeric]=[2, 1, 1])->np.ndarray:
        """Generate multiple boards."""
        boards = []
        for _ in range(n_boards):
            self.fill(n_steps, pushpullnone_ratios)
            boards.append(self.convert_to_jumanji(route=False))
        return np.array(boards)

    def return_training_board(self, key)->np.ndarray:
