...
def create_stack(max_size: int, num_features: int) -> Agent:
    ...
    """
    return Agent(jnp.zeros((max_size, num_features), dtype=int)-1, 0, max_size-1)

def stack_push_head(stack: Agent, element: chex.Array) -> Agent:
...
def stack_push_tail(stack: Agent, element: chex.Array) -> Agent:
    """Push an element on top of the stack.
...
def stack_pop_tail(stack: Agent) -> Tuple[Agent, chex.Array]:
    """Pop the last element from the stack.
...
def empty_stack(stack: Agent) -> bool:
